...
Steinberg~\cite{Steinberg:2025:WTG} recently proposed a wave optics simulator based on cone tracing of Gaussian beams (instead of ray tracing) that has similar structure to a (bidirectional) path tracer. Can we make their model differentiable? What are the computational challenges? What are the applications?
...
\paragraph{Continuous gradient domain rendering.}
Gradient domain rendering~\cite{Kettunen:2015:GPT} typically estimate difference between pixel colors as correlated sampling to reduce variance. This can be seen as estimating finite difference gradients. Can we instead use the continuous screen space gradients instead of the finite differences for the reconstruction? Mitchell discussed some version of this derivative reconstruction theory~\cite{Mitchell:1988:RFC} back in 1988, but did not propose a practical rendering algorithm. 
